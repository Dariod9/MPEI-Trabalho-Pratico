...
public class AppUtilities {
        ...
        for (Entry<Integer, Integer> entry : DatabaseUtilities.numberCount().entrySet()) {
            for (int i = 0; i < 5; i++) {
                if (entry.getValue().equals(freqNum.get(i))) {
                    ...
                    index++;
                    break; // Só pode adicionar um número
                }
            }
            if (index == 5) {
                break;
    ...
    protected static boolean validDate(int day, int month, int year) {
        //o ano�m�nimo considerado v�lido � 2018
    ...
    private static boolean anoBissexto(int y) { //fun��o protegida que verifica se um ano � bissexto
